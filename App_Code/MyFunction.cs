...
namespace My
{
...

    {
...

        {
            ...
            ((Page) HttpContext.Current.CurrentHandler).ClientScript.RegisterStartupScript(typeof(string), key, script,
                true);
        ...
        static public string CustomPageSQL(int CurrentPage, int PageCount, string PrimaryKey, string SelectField,
            string JoinString, string whereString)

        {
            ...
            string TempSqlString =
                "SELECT top ##PageCount## ##SelectField## FROM  ##JoinString## where ##PrimaryKey## Not IN(SELECT top ##TopCount##  ##PrimaryKey## FROM  ##JoinString## where 1=1 ##whereString##) ##whereString##";
...

        {
            ...
            string TempSqlString =
                "SELECT   COUNT(##PrimaryKey##) AS Number FROM ##JoinString## WHERE 1=1 ##whereString##";
        ...
        static public void GenerateThumbnailImage(string name, System.IO.Stream source, string target, string suffix,
            int MaxWidth, int MaxHight)
        ...
        static public void GenerateThumbnailImage(int MaxWidth, string name, string source, string target,
            string suffix)
...

        {
            ...
            byte[] key =
            {
                168, 159, 239, 4, 198, 215, 9, 253, 248, 56, 191, 68, 140, 68, 230, 130, 27, 162, 182, 240, 52, 116,
                130, 18
            };
...

        {
...

            {
                ...
                paginationBuilder.Append(page > 1
                    ? string.Format("<a href=\"{0}page={1}\">上一頁</a>", targetpage, prev)
...

                {
...

                    {
                        ...
                        paginationBuilder.Append(counter == page
                            ? string.Format("<span class=\"current\">{0}</span>", counter)
                            : string.Format("<a href=\"{0}page={1}\">{1}</a>", targetpage, counter));
                ...
                paginationBuilder.Append(page < counter - 1
                    ? string.Format("<a href=\"{0}page={1}\">下一頁</a>", targetpage, nextPage)
